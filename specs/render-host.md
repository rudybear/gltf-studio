...
- [RH-035] (active) Audio viewport helpers (the follow-up RH-032's own doc comment anticipated): `engine-three` additionally recognizes `"audio-emitter"` and `"audio-zone"`. For `"audio-emitter"`, `nodeIndex` resolves against the node's own `extensions.KHR_audio_emitter` binding — singular `.emitter` or PR #59's multi-emitter-per-node array `.emitters`, EVERY bound emitter drawn (not just the first) — and each bound emitter grows its own shape: a translucent range sphere sized from the positional distance model's `refDistance`/`maxDistance` for a `type: "positional"` emitter (additionally a cone volume using `positional.coneInnerAngle`/`coneOuterAngle`, radians, when `positional.shapeType === "cone"`, oriented along the node's local -Z per this project's existing forward convention), or a small speaker glyph for a `type: "global"` emitter. For `"audio-zone"`, `nodeIndex` resolves against the node's own `extensions.KHR_audio_environment.shape` (present only for a ZONE binding, not a LISTENER binding on that same extension slot, `specs/ux-inspector.md` UX-421/422) — a translucent sphere/box volume matching the shape's own `radius`/`size`. Every audio-helper shape shares one visual language (a translucent, non-depth-writing fill paired with a fully-opaque solid-color wireframe outline in a kind-specific hue — cyan for `"audio-emitter"`, violet for `"audio-zone"` — distinct from a light helper's own light-colored wireframe) and, since neither has a live three.js object of its own the way a light does, is added under `editorHelperGroup` with a per-frame `update()` that re-copies the referencing node's current world position/orientation (not scale) — the same "helper lives in the shared overlay group, RH-033/RH-034 apply identically" treatment `"light"` already gets.
