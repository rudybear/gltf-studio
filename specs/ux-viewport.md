...
### Studio lighting and light helpers (full punctual-light control)
...
- [UX-314] (active) The viewport's right-side toolbar has a "Light helpers" toggle (`✺`) that, when on, shows a helper visual (a wireframe icon sized/shaped per light type — cone for spot, sphere-ish for point, arrow for directional, via `RenderHost.setEditorHelpers`'s `"light"` kind, `specs/render-host.md`'s `RH-032`) for EVERY light node in the document; when off, no light gets a helper from this toggle alone. Independent of the toggle's state, the CURRENTLY SELECTED node's own helper is always shown when that selection is a light node — deselecting it (or selecting a non-light node) removes that one helper again unless the toggle is separately on. These helpers are purely an editor-time viewport aid: `RH-034` guarantees they are never written to the document JSON, so importing a document, toggling either state of this control, and exporting again produces byte-identical output.
