...
### Studio lighting and editor helpers (full punctual-light control; generalized to audio emitters/zones by the audio viewport helpers follow-up)

- [UX-313] (active) The viewport's right-side toolbar has a "Studio lighting" toggle (`💡`) controlling `ThreeRenderHost`'s neutral studio rig (a hemisphere + directional key light that lets an unlit asset still render something, `specs/render-host.md`'s own Implementation notes). Its default is AUTO, recomputed every time a document loads or reloads: DIMMED (to a small fraction of its full strength — never fully hidden) when the document carries at least one real `KHR_lights_punctual` light, so an authored scene's own lighting reads as the dominant visible truth without the neutral rig washing it out additively, and FULL STRENGTH when the document has none. Deliberately dims rather than disables entirely: a document whose only authored light is a token/placeholder one with no real illuminating effect (e.g. co-located with the very surface it nominally lights, a genuinely degenerate case) would otherwise go completely dark the instant it's imported — a real regression this project's own e2e fixture (`e2e/global-setup.ts`'s "KeyLight") surfaced during this feature's own testing. Clicking the toggle manually overrides the CURRENT scene's rig strength (`ThreeRenderHost.setStudioLightingEnabled` — `true` full, `false` dimmed); the override does not survive the next reload — AUTO recomputes fresh every time regardless of any earlier manual choice, a deliberate v1 simplicity trade-off (not an oversight) documented here and in `specs/render-host.md`'s own implementation note for this behavior.
- [UX-314] (active) The viewport's right-side toolbar has ONE "Helpers" toggle (`✺`, `data-testid="viewport.helpers-toggle"`) governing EVERY editor-helper kind together — not a toggle per kind. When on, it shows a helper visual for every light node (a wireframe icon sized/shaped per light type — cone for spot, sphere-ish for point, arrow for directional, via `RenderHost.setEditorHelpers`'s `"light"` kind, `specs/render-host.md`'s `RH-032`), every audio-emitter-bound node, and every audio-zone node (`"audio-emitter"`/`"audio-zone"`, RH-035) in the document at once; when off, no node gets a helper from this toggle alone. Independent of the toggle's state, the CURRENTLY SELECTED node's own helper is always shown for whichever kind(s) that selection actually is (a node can be, at most, one of light/audio-emitter/audio-zone in practice, but the union logic makes no such assumption) — deselecting it (or selecting a node with no helper-eligible extension) removes that one helper again unless the toggle is separately on. These helpers are purely an editor-time viewport aid: `RH-034` guarantees they are never written to the document JSON, so importing a document, toggling either state of this control, and exporting again produces byte-identical output.
