...
- [UX-608] (active) Resolves OPEN(UX-audiograph-palette-tbd): the audio-graph canvas has its OWN node-creation palette (distinct from the behavior graph's `specs/ux-graph-canvas.md` `UX-500` palette, since audio nodes come from the ratified `KHR_audio_graph` node-kind list, not `@gltfi/kernel`'s op registry), organized into categories with a search box, mirroring the behavior-graph palette's collapse/search/category-list UX. Clicking a palette entry appends a new node of that kind (with the kind's schema-default params) onto the canvas, scaffolding `extensions.KHR_audio_graph` first if the document has none yet. **r2 update**: the palette is now THREE categories (Filters, Dynamics & Shaping, Channel Routing) — the original "Generators" category, which held only `oscillator`, is gone: r2 (spec PR #2632 @ c0042d7f) removed `oscillator` from the node `oneOf` entirely, moving it to `KHR_audio_emitter` source data (`specs/ux-inspector.md`'s Sources sub-list, `UX-420`/`UX-424`) — this canvas's palette can no longer create one at all. `audio-node-registry.ts` (the palette's data source) is now itself DERIVED from `@gltf-audiograph/kernel`'s registry rather than a hand-copied second catalog of the same schema.
...
- [UX-610] (active) The selected node's params (`KHR_audio_graph.node.schema.json`'s per-kind param set — e.g. a filter's `frequency`/`qualityFactor`, a gain's `gain` number) are editable inline in the node-details panel: a numeric field for a number/integer param, a dropdown for an enum param (e.g. gain `interpolation`, a channel node's `channelInterpretation`), a checkbox for a boolean param. Committing an edit updates `graph.nodes[nodeIndex].params[key]` as one undoable step; rapid successive edits to the SAME param (e.g. dragging a number field) coalesce into a single undo step, matching `specs/ux-graph-canvas.md`'s node-drag coalescing convention. **r2 update**: the oscillator `type` waveform enum this entry originally cited is no longer a `graph.nodes[]` param at all — it moved to a `KHR_audio_emitter` source's `extensions.KHR_audio_graph.oscillator` bag, edited by the SAME field-widget machinery (`AudioParamField`/`isParamFieldVisible`) one level over, in the Audio Emitter inspector's Sources sub-list (`specs/ux-inspector.md` `UX-420`/`UX-424`), not this canvas's node-details panel.
...
- [UX-615] (active) A `splitter` node's output fan and a `channelmerger` node's input fan are addressable beyond slot 0: `audio-node-registry.ts` adds a `numberOfOutputs` param to `splitter` (symmetric with `channelmerger`'s pre-existing `numberOfInputs`, already accepted before this requirement) — an implementation-defined `params` key (the ratified `KHR_audio_graph.splitter.schema.json`/`.channelmerger.schema.json` declare no explicit count param, but the vendored AudioGraphJS runtime's `createChannelSplitter`/`createChannelMerger` read exactly these keys, and `params` has no `additionalProperties: false`). The canvas seeds that many rendered ports for the node BEFORE any connection uses them (`map-audio-graph.ts`'s `defaultPortSlots`), so a splitter/merger's Nth channel port is draggable/connectable as soon as its count param says so, not only once an already-authored document's `connections[]` happens to reference that index (the v1 gap this resolves). `@gltf-studio/audio-graph`'s `validateGraph` additionally flags (error severity) a `connections[]` entry whose output/input index exceeds the node's declared count — a genuine authoring bug (real Web Audio `connect()` would fail at that index).
  **r2 update (spec rules 9/10)**: this entire authored-arity design is GONE. `numberOfOutputs`/`numberOfInputs` are no longer params at all — `@gltf-audiograph/kernel`'s registry has neither, and its `ports.ts` reports both a splitter's output arity and a channelmerger's input arity as `Infinity` (unbounded, definitionally in range) since the real count is DERIVED from the highest port index referenced in `connections[]`/`inputs[]`. There is nothing left to author, so `audio-param-panel.tsx` shows no channel-count field for either kind any more (only `channelInterpretation` remains). The chicken-and-egg problem UX-615 originally solved (a fresh node needs a draggable port before any wire exists) is now solved differently: `map-audio-graph.ts`'s `growFanPorts` always renders exactly one extra, not-yet-wired "next" port past the highest currently-used index, so the canvas stays draggable-to-grow without an authored count. `validateGraph`'s `channel-port-out-of-bounds` check is gone with it — there is no longer a "declared count" a connection could exceed.
...
- [UX-618] (active) Fuller per-node-kind param coverage from the ratified `KHR_audio_graph.node.schema.json`'s per-kind param objects, added to `audio-node-registry.ts`/`audio-param-panel.tsx`: an oscillator's `periodicWave` (`{real: number[], imag: number[]}`, the Fourier-coefficient payload gap-analysis G2 asked for — now real in the ratified schema) and a gain's `curve` (a sampled interpolation curve, also G2) are editable as textarea(s) of comma/whitespace-separated numbers, each shown only when its OWN controlling field takes the relevant value (`type: "custom"` / `interpolation: "custom"` respectively — the new `AudioParamField.showIf` mechanism, `isParamFieldVisible`). A waveshaper's `curve` (an explicit shaping curve the vendored runtime already fully applies, unlike the two above — see UX-619's runtime-vs-schema distinction) gets the same textarea editor, unconditionally visible. Both new field types are `optional: true` — left out of a freshly-added node's initial `params` bag (`defaultParamsFor`) rather than forced in with a placeholder value, since an empty/default `curve`/`periodicWave` could violate the schema's own `minItems` constraints.
  **r2 update**: the oscillator half of this entry moved OFF `graph.nodes[]` — `periodicWave` is now a field of a `KHR_audio_emitter` source's `extensions.KHR_audio_graph.oscillator` bag (`OSCILLATOR_SOURCE_FIELDS`/`isOscillatorSourceFieldVisible` in `audio-node-registry.ts`, the identical `showIf`/textarea mechanism one level over), edited in the Audio Emitter inspector's Sources sub-list (`specs/ux-inspector.md` `UX-420`/`UX-424`) rather than this canvas's node-details panel. Gain's `curve` and waveshaper's `curve` are unaffected by r2 (both stay `graph.nodes[]` params) and this entry's description of them is unchanged.
- [UX-619] (active) Extends `specs/ux-audio-graph.md`'s existing lint-banner/badge machinery (UX-602/609) with explicit RUNTIME-gap messaging, distinct in kind from the DAG-only/envelope SPEC-gap messaging those requirements already cover: `compressor` — gap-analysis G1's "no DynamicsCompressorNode" finding — is now a REAL kind in the ratified schema (added by the same PR #2572 review-fixes refresh that also added oscillator `periodicWave`/gain `curve`, UX-618) and `audio-node-registry.ts`'s palette offers it (Dynamics & Shaping category, the schema's threshold/knee/ratio/attack/release params), but the project's vendored `audio-graph-js` runtime has no `'compressor'` case in its node builder — a genuine IMPLEMENTATION gap, not a spec gap. `@gltf-studio/audio-graph`'s `validateGraph` flags a `compressor` node with a warning naming this distinction explicitly ("valid per the ratified KHR_audio_graph schema... but this project's vendored AudioGraphJS runtime... doesn't implement it yet"), and `AudioGraphHost.audition()` degrades gracefully (traces the build failure, does not throw) rather than crashing the canvas when a document reaches one.
  **r2 update**: the vendored runtime was refreshed to an r2-capable build (re-vendored from the sibling `glTF-audio/AudioGraphJS` repo's `feature/environment-v2` branch @ `12dc5bc`) as part of this pass — `compressor` is STILL genuinely unimplemented there (its `NodeKind` union / `buildGraph` switch has no `'compressor'` case), so this entry's runtime-gap warning is unchanged and still fires. Oscillator `periodicWave`, by contrast, IS now genuinely implemented by the refreshed runtime (`nodes/oscillator.ts` builds a real `PeriodicWave` from it) — the "oscillator `periodicWave` is RUNTIME-unimplemented" half of this entry's original G2 cross-reference no longer applies at all (removed; see `validators.ts`'s header comment) — and the check itself moved to a source-side one (`custom-oscillator-undefined`, `@gltf-studio/audio-graph`'s `validateGraph`), since `periodicWave` is `KHR_audio_emitter` source data under r2, never a `graph.nodes[]` param.
- [UX-620] (active) Card-legibility parity (bug report: "audio graph nodes have no values/parameters, only sockets" — every real node's card previously showed nothing beyond its op label/index/ports, since `map-audio-graph.ts` always set `literals: {}` and no other card row ever summarized `params`; the SELECTED node's params were always correctly editable via `UX-610`'s panel, but nothing on the unselected card hinted at their values — mirrors `specs/ux-graph-canvas.md`'s `UX-512`/`UX-514` card-legibility audit on the interactivity canvas): every real audio-graph node's card renders one additional row below its subtitle, summarizing its CURRENTLY VISIBLE (`isParamFieldVisible`-gated, so a `showIf`-hidden field like gain's `curve` before `interpolation` is `"custom"` is correctly omitted) non-`curve`/`periodic-wave` params as `Label: value` pairs joined with " · " (e.g. `Gain: 0.6 · Interpolation: linear · Duration: 0 s`, `Frequency: 350 Hz · Q: 1`) — `curve`/`periodic-wave` fields are excluded (unbounded arrays, not a card-legible scalar). A param's OWN registry `label` supplies both the short name and, when the label declares one in parentheses (e.g. `"Frequency (Hz)"`), a unit suffix — `audio-node-registry.ts`'s `audioNodeCardSummary` is the ONE place this formatting happens, shared by nothing else (the panel keeps rendering full field labels, e.g. `"Frequency (Hz)"`, unchanged). The row is CSS-ellipsis-truncated with the full text in a hover title, same treatment `subtitle` already gets — a `compressor`'s 5-param summary is long enough to routinely truncate. A freshly palette-added node summarizes immediately and identically to an imported one, since `defaultParamsFor` already fills every non-optional field before the node reaches the canvas at all — there is no "blank until you touch it" gap. The two synthetic terminal kinds (`audio-buffer-source`/`emitter`, `UX-607`) never render this row (no `params` bag to summarize). Implemented via a new, generic optional `MappedNode.configLine` field (`@gltf-studio/graph-canvas`'s `map-graph.ts`) — computed PURELY from the node's own `kind`/`params` in `mapAudioGraph` itself (no document-level lookup needed, unlike `UX-512`'s scene-node-name resolution) — rendered as its own row by the shared `OpNode`/sized by `estimateNodeSize` (`elk-layout.ts`, so the card's actual box accounts for the extra row instead of it silently overflowing). No `KHR_interactivity` op sets `configLine` today; the field is named for what it shows, not which canvas populates it.
