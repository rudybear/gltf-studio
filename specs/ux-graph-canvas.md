...
### Debugger integration (D2, specs/ux-debugger.md UX-1505 block)

- [UX-522] (active) A graph node whose resolved emitted-script line currently holds a session
  breakpoint (`specs/ux-debugger.md` `UX-1505`) shows a small red corner badge (`gcanvas.breakpoint-
  badge.{index}`) — a DIFFERENT corner from `UX-506`'s validation badge, so both can render on the
  same node at once. Full behavior contract (resolution, honest gaps) lives in `specs/ux-debugger.md`
  `UX-1506`, per this file's own "entry point here, content in the owning file" split (mirrors how
  `ux-debugger.md` itself already treats `specs/ux-shell.md` `UX-130`'s play-bar toggle).
- [UX-523] (active) The node-details card (`UX-507`) gains a "Break here" button (`gcanvas.details
  .break-here`) — disabled, with an explanatory tooltip, when the selected node has no corresponding
  script line. Full behavior contract in `specs/ux-debugger.md` `UX-1507`.
...
## Implementation notes (shared `MappedNode.configLine`, consumed by `@gltf-studio/audio-canvas`'s UX-620)

`map-graph.ts`'s `MappedNode` gained one more small, generic, OPTIONAL field alongside `handlerTarget`/`subtitleMissing` above: `configLine?: string` — an already-formatted single-line summary a caller wants rendered as its own card row, CSS-ellipsis-truncated with the full text in a hover title (the exact treatment `subtitle` already gets). `op-node.tsx` renders it unconditionally when set (a plain `<div className="gcanvas-op-config-line">`, no category/op gating — unlike `target`/`clip:` rows, which are gated to specific interactivity ops), and `elk-layout.ts`'s `estimateNodeSize`/`estimateWidth` account for the extra row/width so a card that sets it doesn't silently overflow its ELK-estimated box. No `KHR_interactivity` op sets this field today — it exists purely so `@gltf-studio/audio-canvas`'s card-legibility fix (`specs/ux-audio-graph.md`'s `UX-620` — audio-graph node cards previously showed ports only, never their `params`) has a generic row to populate `mapAudioGraph`'s own real nodes with, computed purely from each node's `kind`/`params` (no document-level lookup needed, unlike `handlerTarget`'s scene-node resolution). This package's own behavior is otherwise unchanged: every existing `graph-canvas.spec.ts` assertion (including the port-handle/label-overlap regression suite, which is sensitive to exactly this kind of card-height change) still passes untouched.
