...
- [AH-003] (active) `WebAudioHost` (concrete class only — `AH-002`'s interface is unaffected) gains a constructor-only `WebAudioHostOptions.onSuspendedTrigger?: (message: string) => void` — the same "non-interface extension point" shape `resolveAudioUri` already established. It fires, at most once per `applyPointer` call, when a `/extensions/KHR_audio_emitter/sources/{i}/playing` one-shot trigger pointer is applied with a truthy value while `this.context` is either absent (`init()` never called) or exists but is not `"running"` — i.e. exactly the cases under which the trigger would otherwise have zero audible effect with no observable signal anywhere. Checked BEFORE `applyPointer`'s own `!this.context` early-return, so the "no context at all" case (not just "context exists but suspended") is covered. This is a fallback diagnostic, not the fix for the bug it was added alongside — `specs/ux-shell.md`'s `UX-131` (arming the context at Play-start, in the same gesture as the Play click) is the actual fix; a correctly-armed session should hit this hook rarely if ever.
...
- Fix: the champagne starter's pop was silent on a fresh Play (`AH-003`, `specs/ux-shell.md` `UX-131`). Root cause, characterized against the built app in both headless and headed Chromium before any fix landed: nothing on the Play path ever called `AudioHost.init()` — the ONLY call site was the Inspector's Audition button (`specs/ux-inspector.md` UX-406) — so a user who loaded a starter and went straight to Play without ever auditioning had `WebAudioHost.getDiagnostics()` reading `"audio idle"` (i.e. `this.context === null`, not merely suspended) for the entire session; the click-to-pop interaction itself worked fine end-to-end (the graph fired, `popped` flipped, geometry animated) because `applyPointer`'s pointer fan-out (PC-001) runs unconditionally against both hosts — only the audio side of the trigger silently no-opped against `applyPointer`'s pre-existing `if (!this.context) return;` guard. This had gone uncaught because every existing e2e path that reaches Play (`e2e/champagne.spec.ts`, `e2e/golden-path.spec.ts`) always clicks Audition first, itself a valid user gesture that arms the context before play mode ever starts. Fixed at the `packages/app` layer (`UX-131`): `startPlay()` now calls `audioHost.init()` itself, synchronously in the Play button's own click call stack (before `startPlay()`'s first `await`), unconditionally (not gated on "does this document declare `KHR_audio_emitter`" — a running-but-silent `AudioContext` is free, and cheaply detecting "has audio" would still miss an emitter a live `KHR_interactivity` edit adds mid-session); the in-play viewport click path (`Viewport.tsx`'s `onClick`, PC-008's fan-out) re-arms the same way as a fallback, since a click during play is itself a fresh gesture too. `WebAudioHost.init()`'s own create-or-resume idempotency (unchanged) makes both call sites safe to invoke unconditionally regardless of whether an earlier Audition gesture already armed the same host. `AH-003`'s `onSuspendedTrigger` hook exists for whatever gap remains after both arming points — surfaced by `packages/app` on its own Console tab (`App.tsx`), not inside this package.
