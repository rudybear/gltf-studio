...
- M7 audio-graph gaps closed + deeper runtime (audio pass 1-2/3, `specs/ux-audio-graph.md`'s `UX-615`..`619`): `AudioGraphJsHost.audition()` now catches a `buildGraph()` failure (concretely: a graph reaching a `compressor` node — schema-valid `KHR_audio_graph` per the ratified schema's PR #2572 review-fixes refresh, but the vendored `audio-graph-js@0.1.0` runtime has no builder for it yet) and traces it rather than throwing — this is `AGH-lint-shape-tbd`'s resolution's own anticipated `"build-failed"` code's first real use (previously documented as an anticipated `code` value but never actually produced anywhere). `AGH-001`'s lint surface also gains four new `code`s from `@gltf-studio/audio-graph`'s `validators.ts`: `compressor-runtime-unimplemented`, `oscillator-periodicwave-runtime-unimplemented`, and `gain-curve-runtime-unimplemented` (all `"warning"` severity — schema-valid, not yet built/applied by this project's vendored runtime, explicitly NOT the same as gap-analysis G1/G2's original "spec gap" framing, which the ratified schema has since resolved), and `channel-port-out-of-bounds` (`"error"` severity — a genuine document bug: a `splitter`/`channelmerger` connection referencing a channel index beyond its declared `numberOfOutputs`/`numberOfInputs`, which would fail at real Web Audio `connect()` time). None of these change `AGH-001`'s interface shape (still `{ graphIndex, severity, code, message, nodeIds }`) or `AGH-buildgraph-signature-tbd`/`AGH-audition-signature-tbd`'s resolutions above.
