...
- [UX-131] (active) Play-start audio-arming policy (fix for the champagne starter's silent cork pop): the `playbar.play` button's click is itself the user gesture `specs/engine-api.md`'s `AH-001` requires — `startPlay()` (`store/app-store.ts`) calls `audioHost.init()` synchronously within that same click's call stack, before its own first `await`, so the browser's autoplay-gesture check still sees the click on the stack when the `AudioContext` is created/resumed. Armed UNCONDITIONALLY on every Play, regardless of whether the current document declares `KHR_audio_emitter` — a running-but-silent `AudioContext` is harmless, and there is no cheap, always-correct "this document has audio" check (a live `KHR_interactivity` edit can add an emitter mid-session). This does not replace the Inspector's own Audition-gesture arming (`specs/ux-inspector.md` UX-406) — either can arm the same host first; `WebAudioHost.init()`'s create-or-resume idempotency makes calling it from both, or twice, harmless. Belt-and-suspenders: the in-play viewport click path (`Viewport.tsx`'s `onClick`, routing to `fireSelect` per `specs/engine-api.md` PC-008) re-arms the same host on every click while playing/paused — also a real gesture, and cheap to re-check since `init()` is a no-op once already running. `specs/engine-api.md`'s `AH-003` (`WebAudioHost`'s `onSuspendedTrigger` option, wired to this file's own Console tab via `App.tsx`'s `log("warn", ...)`) is the honesty fallback for whatever gap remains after both arming points — a one-shot audio trigger firing while still unarmed is reported there instead of going silently unheard.
...
- [UX-120] (active) Supersedes UX-119: the viewport's empty-project state (`UX-3xx`'s "no document open" placeholder) shows a starter-experience gallery (`viewport.gallery`) of cards instead of a single button, alongside the existing "Import a .glb to get started" note. There are exactly three cards, each with a small static preview, a short one-sentence description, and a Load control. The FIRST card is Empty scene (`viewport.gallery.card.empty`, card copy "Empty scene — start from scratch; use + Add to build."): clicking its Load control builds a minimal document entirely in memory — an `asset` header plus one default scene with ZERO nodes, no fetch involved (`packages/app/src/lib/empty-scene.ts`'s `buildEmptySceneGlb`, a real `.glb` produced via `@gltfi/gltf`'s own `writeContainer`) — then imports those bytes through the exact same path a manually-picked file would (`importGlb`), so the result is a real, storage-persisted, undo-historied project from the first tick, not a special-cased blank-slate mode. The SECOND card, R4 Racer (`viewport.gallery.card.racer`, `samples/r4-racer.glb`, described in-card as "a complete racing game authored as TypeScript, compiled into the asset; click the pads to steer"), is unchanged from `UX-119`: clicking its Load control fetches that card's asset as a static file this app's own build serves and imports it the same way. The THIRD card, Champagne (`viewport.gallery.card.champagne`, `samples/champagne.glb`, described in-card as "Pop the cork — click it in Play mode. Then open its brain in the graph and script tabs."), fetches and imports its asset the same way as R4 Racer — a procedurally generated bottle/cork/foam-burst scene (`scripts/make-champagne.mjs`, the source of truth `pnpm champagne` builds and verifies) whose `KHR_interactivity` behavior graph is itself authored as real TypeScript ("GIscript") compiled via `@gltfi/parse-ts`'s `parseModule` + `@gltfi/ir`'s `exportGraph`, the same pipeline the Script tab's Apply button drives on a live document — a deliberate showcase, unlike R4 Racer's much larger externally-vendored asset, that a curious user can open in this app's own graph canvas and Script tab and read start to finish. Clicking Cork in Play mode fires its `event/onSelect`: a positional `KHR_audio_emitter` pop sound, a staggered `KHR_node_visibility` foam-sphere reveal, and a two-phase `pointer/interpolate` launch-and-tumble arc for Cork itself (plus a small independent bottle recoil); a second click once the arc settles interpolates Cork back to its exact authored rest pose and re-hides the foam, replayable indefinitely. A failed R4 Racer or Champagne fetch surfaces a toast (`UX-109`) naming the card's label, rather than a silent no-op. Card testids use a semantic `<key>` (`empty`/`racer`/`champagne`), not a numeric index — the same non-numeric-but-repeated-part pattern `UX-110`'s own convention note permits and `specs/ux-viewport.md`'s play-overlay variable rows (`viewport.play-overlay.variable.<key>`) already establish. Zero-node tolerance (verified for the Empty scene card, and true generally of every surface that reads `document.json`): the scene tree shows a dedicated empty note (`scene-tree.empty-scene`, `specs/ux-scene-tree.md`'s owned surface — distinct from the no-document `scene-tree.empty`) instead of a bare blank list; the viewport renders its usual chrome with no geometry and no crash (`buildRenderScene`'s existing `!json.meshes` short-circuit already covered this); the Behavior graph tab shows `specs/ux-script.md` `UX-714`'s existing no-graph empty state, since the empty scene carries no `KHR_interactivity` extension at all; and both the scene tree's `+ Add` (creates the document's first node immediately — `SceneEdit.addNode`'s existing missing-array-creation fallback needed no changes) and Export (`UX-112`, a real, valid, zero-node `.glb`) work with no special-casing anywhere. `samples/playground.glb` and `scripts/make-sample.mjs` (the retired card's asset) are kept as a test-only fixture, not shipped in the built app: `e2e/golden-path.spec.ts` loads it directly through the top bar's Import control (`topbar.import-input`) instead of a gallery card, and `copy-sample.mjs` no longer copies it into `packages/app/public/` (it does copy both `r4-racer.glb` and `champagne.glb`).
...
- Champagne starter (`UX-120` r2 — three cards now, not two; `Viewport.tsx`,
  `SampleGalleryPreviews.tsx`'s new `ChampagnePreview`, `packages/app/scripts/copy-sample.mjs`,
  `samples/README.md`, `scripts/make-champagne.mjs` (new), `e2e/shell.spec.ts`,
  `e2e/golden-path.spec.ts`, `e2e/champagne.spec.ts` (new)): a third gallery card, alongside Empty
  scene and R4 Racer — a procedurally generated champagne bottle whose cork pops open on click. This
  is the transpiler pipeline's own showcase card: unlike `make-sample.mjs`'s hand-authored JSON
  graph, `make-champagne.mjs`'s `KHR_interactivity` graph is written as real TypeScript ("GIscript")
  and compiled via `@gltfi/parse-ts`'s `parseModule` + `@gltfi/ir`'s `exportGraph` — the exact
  pipeline the Script tab's Apply button drives on a live document (`packages/script-panel/src/
  equivalence.ts`) — then validated (`@gltfi/verify`'s `validateGraph`) and proven behaviorally (a
  headless `@gltfi/runtime` interpreter run firing `onSelect` on Cork twice: pop, then reset) before
  the script ever writes the `.glb` to disk. The graph itself: a `popped`/`animating` variable pair
  gates a single `event/onSelect` on Cork — first click triggers the audio-emitter "playing" pointer,
  a staggered `KHR_node_visibility` reveal of six foam spheres, a two-phase `pointer/interpolate`
  arc (ease-out up-and-sideways chained via its own `done` continuation into an ease-in
  down-and-further-sideways "fall", giving a gravity-arc feel from just two interpolates) with a
  matching two-phase rotation tumble around one fixed axis, plus an independent small bottle recoil
  tilt-and-back; a second click once `animating` clears interpolates Cork back to its exact authored
  rest pose and re-hides the foam, `popped` clearing last — replayable indefinitely, and any click
  while `animating` is a no-op. See `e2e/champagne.spec.ts` for the real-viewport-click coverage
  (interpreter AND compiled-engine parity) and `scripts/make-champagne.mjs`'s own header comment for
  the full scene layout and geometry notes (a hand-rolled ring-segment lathe for the bottle/cork
  profiles, a synthesized three-layer "cork pop" WAV, deterministic via a seeded PRNG).
