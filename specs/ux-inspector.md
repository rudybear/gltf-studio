...
- [UX-420] (active) The Audio Emitter section additionally shows a Sources sub-list: one row per index in the emitter's own `sources: number[]` array, naming the bound clip (`extensions.KHR_audio_emitter.audio[source.audio].mimeType`/`uri`, read-only — clip REPLACEMENT/upload is out of scope, mirroring `UX-416`'s texture-slot precedent) alongside editable Gain/Playback Rate/Loop/Autoplay fields, written via the new `SceneEdit.setAudioSourceProperty` (`specs/document-model.md`'s `DOC-062`). **r2 update**: each row leads with a Source Type select (Clip/Oscillator, `UX-424`) — Playback Rate/Loop are shown only in Clip mode (r2 declares both ignored for an oscillator source).
...
- [UX-424] (active) r2's oscillator authoring surface: each `UX-420` Sources sub-list row leads with a Source Type select — "Clip" (the original `audio`-indexed shape) or "Oscillator" (`extensions.KHR_audio_graph.oscillator`, no `audio` index, `KHR_audio_graph` r2's node-kind removal — see `specs/ux-audio-graph.md`'s `UX-608`/`UX-610`/`UX-618` r2 updates). Switching modes wholesale-replaces the source object (`SceneEdit.setAudioSourceProperty(…, [], …)`), preserving the two schedule fields both shapes share (Gain, Autoplay — r2's `SOURCE_SCHEDULE_KEYS`) and dropping the rest; switching to Clip seeds `audio: 0` when the document has at least one audio clip (this section still has no clip-picker UI, `UX-420`'s existing "no clip replacement" scope). In Oscillator mode, Playback Rate/Loop are hidden (r2 declares both ignored for an oscillator source) and the row instead shows Waveform/Frequency/Detune/Pulse Width fields (`@gltf-studio/audio-canvas`'s `OSCILLATOR_SOURCE_FIELDS`, itself derived from `@gltf-audiograph/kernel`'s `OSCILLATOR_SOURCE_SPEC` — the SAME registry the Audio Script tab's transpiler pipeline uses, so this inspector section and the Audio Script tab's `a.oscillator(...)` surface can never drift against each other), plus a Custom waveform (Fourier coefficients) real/imag textarea pair shown only when Waveform is "custom" (`isOscillatorSourceFieldVisible`, the same `showIf` mechanism `specs/ux-audio-graph.md`'s `UX-618` uses one level over for `graph.nodes[]` params). None of these fields route through `WebAudioHost.applyPointer` (`UX-423`'s existing "reload fallback" note covers them the same way) — an edit here re-derives on the next `attachAudioHost` reload rather than patching a live node in place. This is authoring-only: a `KHR_audio_graph` graph's `inputs[]` binding of THIS source into a processing chain is still done on the audio-graph canvas (`specs/ux-audio-graph.md`'s `UX-611`), not here.

### Source/emitter lifecycle + clip assignment (Track A audio task)

- [UX-425] (active) The `UX-420` Sources sub-list's Clip row is no longer read-only: it's a `<select>` listing every document `audio[]` clip by name/uri (`SceneEdit.setAudioSourceProperty(…, ["audio"], …)`), disabled only when the document has zero clips — resolving `UX-420`'s own "clip REPLACEMENT/upload is out of scope" note now that `specs/document-model.md`'s `DOC-066` clip factories exist. The sub-list also gains "+ Add clip source" (`SceneEdit.addSourceToEmitter(…, {audioIndex: 0})`, disabled when there are no clips) and "+ Add oscillator source" (`SceneEdit.addSourceToEmitter(…)`, no `audioIndex`) buttons below the list, and each row gains its own "✕" remove button (`SceneEdit.removeSourceFromEmitter`) — sources are per-emitter and removal is unconditionally safe (the referenced clip itself is untouched, DOC-066's own orphan policy), unlike clip DELETION, which stays blocked-when-used.
- [UX-426] (active) Multi-emitter-per-node, closing `UX-423`'s own documented "a node can carry only ONE emitter through this UI" gap: the Inspector normalizes a node's `KHR_audio_emitter` binding to a list (the singular `.emitter` field, or the array-valued `.emitters` field, exactly like `WebAudioHost` itself already reads both interchangeably) and renders one full Audio Emitter section (`UX-406`/`UX-419`/`UX-420`) per bound emitter, each with its own "✕ Remove this emitter" action in its header (`SceneEdit.removeEmitterFromNode`) — deleting the SOLE emitter leaves the node audio-less (a valid, honest state, not blocked), deleting one of several splices it out of `.emitters` without collapsing a one-remaining-element array back to the singular shape. A "+ Add audio emitter" button below the list (`SceneEdit.addEmitterToNode`) appends another, fresh (silent-WAV-placeholder) emitter binding to the SAME node.
- [UX-427] (active) The Audio Environment Zone section (`UX-421`) gains a Shape select (`sphere`/`box`) alongside the existing Zone Radius field — choosing `box` swaps in three Box Size (x/y/z) number fields instead, all via `SceneEdit.setNodeAudioEnvironmentProperty(…, ["shape"], …)` (a wholesale shape-object replacement, since sphere/box share no fields) — closing `UX-421`'s own "(sphere radius / box size)" wording, which the original pass under-delivered (radius-only; `AudioEnvironmentSection.tsx`'s `setShapeRadius` hardcoded `type: "sphere"` with no box path at all).
- [UX-428] (active) Audition's listener-pose gap (`UX-423`'s own documented note: "Audition itself still previews against the Web Audio API's default listener pose, not the live viewport camera") is PARTIALLY closed: the Audition button now reads the live `RenderHost.getCameraPose()` and pushes it to `AudioHost.setListenerPose` ONCE, immediately before `auditionEmitter` — a snapshot of wherever the viewport camera actually is at the moment Audition is clicked, not a hardcoded origin. This is deliberately NOT a continuous per-frame feed: `Viewport.tsx`'s own `setListenerPose` interval stays gated on `playState === "playing"` only, for the same CI-flake-avoidance perf reason its own doc comment already gives — reinstating a continuous feed for Audition too was explicitly out of scope for this pass. The REMAINING honest gap: a positional emitter auditioned while the camera keeps moving will not re-pan mid-preview, only at the click that started it.
...
- Emitter/environment/listener authoring, audio pass 3/3 (`UX-419`..`423`,
  ...
  Real, deliberately out-of-scope follow-ups, not silently dropped at the time: a node can carry only ONE emitter
...
- Clip management + source/emitter lifecycle (Track A audio task, `UX-425`..`428`,
  `specs/document-model.md`'s `DOC-066`): closes every follow-up the bullet immediately above named.
  Multi-emitter-per-node authoring is real (`UX-426`); the Add menu's "Audio Emitter" entry is now a
  submenu offering "New (silent placeholder)" plus one entry per existing document clip
  (`specs/ux-scene-tree.md`'s own updated `UX-206`), wiring `addAudioEmitterNode`'s `opts.audioIndex`
  at last; the Sources sub-list's Clip field is a real picker (`UX-425`), not read-only; and the Audio
  Environment Zone's shape gains a real box option (`UX-427`). The "environment zone OR listener, never
  both" policy is UNCHANGED — not revisited by this pass. Audition's listener-pose gap is narrowed, not
  fully closed (`UX-428`'s own note: a one-shot snapshot at click time, still no continuous feed).
