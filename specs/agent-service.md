...
### Local OpenAI-compatible provider (docs/adr/0005)

- [AG-017] (active) `packages/agent-llm`'s `OpenAICompatibleAgentProvider` implements `AgentService` (`AG-001`) by sending the assembled prompt and context to a locally-configured OpenAI-compatible `POST /v1/chat/completions` endpoint (e.g. Ollama, LM Studio) and resolving the response's `tool_calls` — never response text — into an operation list; it produces its `Proposal` through the same shared "operations → commands via `GraphEdit.*`/`SceneEdit.*`/`AudioGraphEdit.*` → validate on a scratch document" pipeline `packages/agent-mock` uses (extracted per `AG-003`/`AG-006` so neither provider forks validation logic), never a parallel command-building or validation path of its own.
- [AG-018] (active) The request sent to the endpoint includes a `tools` array of JSON-Schema function definitions — the operation schema — covering a curated subset of `editor-core` command factories; the model's only channel for proposing a change is a `tool_calls` entry naming one of these functions with arguments, never free-form response text interpreted as an edit.
- [AG-019] (active) A response containing no `tool_calls` (prose only, refusal, or an empty choice) is treated as `AgentService.request` rejecting with a refusal error (matching `packages/agent-mock`'s `AgentRequestRefusedError` contract), not as a resolved-but-empty `Proposal`.
- [AG-020] (active) A `tool_calls` entry whose `arguments` fail to parse as JSON, name a function outside the offered schema, or fail the corresponding command factory's own argument validation is dropped from the operation list rather than crashing the request; if zero operations in the batch resolve, the request is a refusal (`AG-019`), and if at least one resolves, the resulting `Proposal` is built (and validated, `AG-006`) from only the operations that did.
- [AG-021] (active) A request that fails at the network layer (connection refused, timeout) or is blocked by the browser as cross-origin (CORS) surfaces as a typed error distinguishable from a refusal (`AG-019`) or a validation failure (`AG-008`), carrying enough information for the UI to show which of the two networking failure modes occurred and the corresponding remediation (endpoint unreachable vs. CORS configuration needed).
- [AG-022] (active) Provider configuration (selected provider, local endpoint base URL, model name, optional API key) is persisted in the browser's own local storage and is read only by the configured provider to address its own requests — it is never transmitted anywhere other than the user-configured endpoint itself, and provider selection defaults to the mock provider (`AG-010`) on first load and in the absence of any stored selection.
- [AG-023] (active) The system prompt built for the endpoint always lists EVERY node in the document by index and name (plus whether it has a `KHR_audio_emitter`), regardless of what's selected or explicitly attached as context, alongside the known `KHR_interactivity` trigger-event declarations (`event/onStart`, `event/onTick`, `event/onSelect`, `event/onHoverIn`, `event/onHoverOut`) a tool's `eventOp` argument can name. Found live against real models (task: real-model validation, 2026-08): a prompt naming an object the user hasn't selected (e.g. "when the pillar is clicked...") got an honest clarification refusal from every tool-calling-capable model tested when the prompt only described the current selection — the model had no index to resolve "the pillar" to. This is the fix, not a parser/response change: `AG-019`'s "the model responded but gave us nothing usable" contract is unchanged, a live model's refusal when genuinely under-informed is still the correct outcome, not a bug to route around.
- Implementation note (real-model validation task, 2026-08): validated live against Ollama (`qwen3.5:122b-a10b`, `qwen3.6:latest`, `gemma4:26b`) via `scripts/ai-smoke.mjs`'s prompt matrix (env-gated, `GLTFI_LLM_LIVE=1`, not part of CI — no GPU there) and one real-browser proof (`e2e/copilot-live-model.spec.ts`, also env-gated). `gemma4:26b` is the recommended default among installed models: on the full matrix (spin/move/play-sound-with-and-without-emitter/add-cubes/multi-step/out-of-scope/ambiguous) it matched the larger `qwen3.5:122b-a10b`'s accept-when-answerable / refuse-when-not pattern exactly, while responding in ~0.5s once warm versus `qwen3.5:122b-a10b`'s 14-40s (its longer "reasoning" preamble adds both latency and token cost) — a `keep_alive`d small tool-calling model beats a slow, correct-but-sluggish large one for an interactive UI. One real per-model divergence worth naming (not a bug): `qwen3.6:latest`, once given `AG-023`'s full node list, became MORE willing to invent an interpretation of the genuinely ambiguous "make it interactive" prompt (spinning multiple untouched nodes) rather than asking for clarification, where `qwen3.5:122b-a10b` and `gemma4:26b` both still asked. Ambiguity handling is inherently a property of the underlying model's judgment, not something this provider enforces beyond `AG-006`'s validation gate and the human Accept step (`docs/adr/0004`) — user config choosing a different model is expected to shift this behavior, which is exactly why `AG-022` leaves model choice to the user rather than this spec hard-coding one. Also confirmed live: `deepseek-r1:32b` (this Ollama build) rejects tool-calling requests outright (`400 ... does not support tools`) — a model-capability mismatch surfaced as an ordinary HTTP failure, not a `NoPlanProducedError`; not every locally-installed model is a valid choice for this provider regardless of prompting.
...
- OPEN(AG-privacy-tbd): prompt/response logging, retention, and third-party data handling are properties of whichever concrete `AgentService` provider is wired up; the mock provider (`AG-010`) has no privacy surface (no network call), so this is deferred rather than resolved. Partially addressed by `docs/adr/0005` for the local-endpoint provider specifically (`AG-017`..`AG-022`): a local endpoint has no third party to disclose a retention policy for, since the request never leaves the user's machine — but this remains open for any future hosted-proxy provider, which would have a real privacy surface this spec still does not pin down.
