...
export function Viewport(): JSX.Element {
  ...
  // specs/ux-shell.md UX-114: the empty-project state's "Load sample scene"
  // affordance -- fetches the checkpoint sample asset (samples/
  // playground.glb, generated by scripts/make-sample.mjs) as a static file
  // this app's own build already serves (packages/app/scripts/
  // copy-sample.mjs copies it into public/ at predev/prebuild, same
  // mechanism as gltfi-runtime-lib.mjs) and imports it exactly the way a
  // manually-picked file would via the top bar's Import control.
  // `import.meta.env.BASE_URL` (not a hardcoded "/") so this also works
  // once deployed under a GitHub Pages project-site subpath.
  async function onLoadSampleScene(): Promise<void> {
    try {
      const response = await fetch(`${import.meta.env.BASE_URL}playground.glb`);
      if (!response.ok) throw new Error(`fetch failed: ${response.status}`);
      const bytes = new Uint8Array(await response.arrayBuffer());
      await importGlb({ name: "playground.glb", bytes });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      pushToast(`Couldn't load the sample scene: ${message}`);
    }
  }
      ...
      >
        {!document && (
          <div className="viewport-empty-state" data-testid="viewport.empty-state">
            ...
            <button className="btn" data-testid="viewport.load-sample" onClick={() => void onLoadSampleScene()}>
              Load sample scene
            </button>
          </div>
        )}
