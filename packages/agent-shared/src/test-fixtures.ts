...
// packages/editor-core/src/test-fixtures.ts's pattern exactly (round-trips
// through @gltfi/gltf's real parseContainer/writeContainer rather than just
// wrapping a JSON string, so tests exercise the same container machinery
// production code uses) — not re-exported from editor-core (that file is
// test-only there too), so duplicated here rather than reached into. Same
// pattern as packages/agent-mock/src/test-fixtures.ts, duplicated rather
// than imported across a package boundary purely for test fixtures.
